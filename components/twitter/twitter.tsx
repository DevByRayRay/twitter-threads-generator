...
const TwitterApp = () => {
	...
	const { fun } = useContext(GeneralContext)
	...
	function sendTweet() {
		...
		sendTweetRequest(fun, tokens, renderedTweets)
			.then((tweetsArr: any[]) => {
				setSend(true)
	...
	return (
		<AppContainer>
			<AppColumn className="column--input">
				...
				<Textarea onChange={onChangeTweet} onKeyDown={onChangeTweet} value={tweet} className="text input"></Textarea>
